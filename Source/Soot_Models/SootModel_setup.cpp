...
SootModel::defineMemberData()
{
  ...
  V0 = m_SootMolarMass/(avogadros*m_SootDensity); // cm^3
  S0 = std::pow(36.*M_PI, 1./3.)*std::pow(V0, 2./3.); // cm^2

  // Soot density (unitless)
  m_SootDensityC = m_SootChi*S0;

  // Factors used throughout
  m_colFact = M_PI*Rgas/(2.*avogadros*m_SootDensity);
  m_colFact23 = std::pow(V0, 2./3.);
  m_colFact16 = std::pow(V0, 1./6.); // Units: cm^0.5
  ...
  for (int i = 0; i != numSootMom; ++i)
    {
      ...
      m_momUnitConv[i] = std::pow(V0, MomOrderV[i])*
	std::pow(S0, MomOrderS[i])*avogadros;
  ...
  m_momUnitConv[numSootMom] = avogadros;
